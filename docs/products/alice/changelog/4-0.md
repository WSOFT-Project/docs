...
Alice4では、大まかに以下の新機能が導入されました。

- **辞書型の導入**: キーと値のペアを保持できる[辞書型](../api/dictionary/index.md)を導入しました。
...
#### Dictionary型
Dictionary型は、キーと値のペアのリストです。Alice 4では、この辞書型に加えて辞書式を導入しました。
次の例をご覧ください。
...
また、簡単にキーと値のペアを表すオブジェクトを作成できる、KeyValuePair式を導入しました。
次の例をご覧ください。
...
この変更は[WSOFT-Project/alicescript#26](https://github.com/WSOFT-Project/alicescript/issues/26)で提案、検討しました。
